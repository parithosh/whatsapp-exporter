...
def query_messages_from_table_messages(con: Connection, key_remote_jid: str, contacts: Dict[str, Optional[str]]) -> List[Message]:
    cur = con.cursor()
    query = """
            SELECT received_timestamp, remote_resource, key_from_me, HEX(data), media_caption, media_wa_type
            FROM messages 
    ...
    try:
      for received_timestamp, remote_resource, key_from_me, data, media_caption, media_wa_type in cur.execute(query, {"key_remote_jid": key_remote_jid}):
          ...
          messages.append(
            Message(received_timestamp, remote_resource, key_from_me, utf8_data, media_caption, int(media_wa_type), contacts.get(remote_resource, None))
          )
...
def query_messages_from_table_message(con: Connection, key_remote_jid: str, contacts: Dict[str, Optional[str]]) -> List[Message]:
    ...
    query = """  
            SELECT
                ...
                m.message_type
            FROM message AS m
            LEFT JOIN chat_list AS cv ON m.chat_row_id = cv._id
...
def query_all_chats(db_path: str, contacts: Dict[str, Optional[str]]) -> List[Chat]:
    ...
    query = "SELECT key_remote_jid, subject, sort_timestamp FROM chat_list WHERE sort_timestamp IS NOT NULL ORDER BY sort_timestamp DESC"
